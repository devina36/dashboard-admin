...
export default function Search({ params }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const response = await fetch(`https://dummyjson.com/products/search?q=${params.id}&limit=${limit}&skip=${skip}`);
  ...
  return (
    <>
      ...
      <h1 className="text-2xl text-bold mb-7">Search: {params.id}</h1>
      <div>
        {loading ? (
          <p>Loading...</p>
        ) : data.total === 0 ? (
